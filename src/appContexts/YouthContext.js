...
import { getBirthDayQuery, getYouthsQuery,updateYouthQuery, createYouthQuery, deleteYouthQuery} from "../graphql/queries";
  ...
  const updateStateAfterUpdateMutation =(newData)=>{
       if(newData){
            setYouth((prevState) => {
              ...
              const oldData = data.find(youth=>youth.id === newData.updateYouth.id)
...
const updateStateAfterDeleteMutation =(newData)=>{
  if(newData && newData.deleteYouth.success){
    setYouth((prevState) => {
      ...
      return  {...prevState,youths:prevState.youths.filter(youth=>youth.id !== newData.deleteYouth.id)};
    });
     
}
};
 //MUTATION HOOKS
  const [updateYouth] = useMutation(updateYouthQuery,{onCompleted:(data)=>updateStateAfterUpdateMutation(data) });
  const [createYouth] = useMutation(createYouthQuery,{onCompleted:(data)=>updateStateAfterCreateMutation(data) });
  const [deleteYouth] = useMutation(deleteYouthQuery,{onCompleted:(data)=>updateStateAfterDeleteMutation(data) });
  ...
  return (
    <YouthContext.Provider value={{ youths,youthsOperations, currentBirthdays }}>
      {children}
